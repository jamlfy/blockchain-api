  ...
  */
	...
	_createClass(chain, [{
		...
		value: function chart(type, callback) {
			var format = 'json';
			this.__Sender(this._url + 'charts/' + type, { format: format }, callback);
		}
	}, {
		key: 'ticker',
   ...
   */
		...
		value: function wallet(guid, pass1, pass2) {
			...
			this.wallet.balance = function (cb) {
				return _this.__Sender(_this._url + 'merchant/' + _this._guid + '/balance', { password: _this._pass1 }, cb);
			};
			...
			this.wallet.newAddress = function (label, cb) {
				return _this.__Sender(_this._url + 'merchant/' + _this._guid + '/new_address', { password: _this._pass1, second_password: _this._pass2, label: label }, cb);
			};
